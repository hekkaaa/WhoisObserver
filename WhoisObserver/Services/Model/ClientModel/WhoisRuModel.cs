namespace WhoisObserver.Services.Model.ClientModel
{
    public class WhoisRuModel
    {
        public string adminc { get; set; }
        public string address { get; set; }
        public string inetnum { get; set; }
        public string counts { get; set; }
        public string comment { get; set; }
        public string created { get; set; }
        public string city { get; set; }
        public string country { get; set; }
        public string netrange { get; set; }
        public string descr { get; set; }
        public string status { get; set; }
        public string stateprov { get; set; }
        public string cidr { get; set; }
        public string netname { get; set; }
        public string nethandle { get; set; }
        public string mntby { get; set; }
        public string parent { get; set; }
        public string nettype { get; set; }
        public string regdate { get; set; }
        public string updated { get; set; }
        public string postalcode { get; set; }
        public string org { get; set; }
        public string orgname { get; set; }
        public string orgid { get; set; }
        public string orgtechhandle { get; set; }
        public string orgtechname { get; set; }
        public string orgtechphone { get; set; }
        public string orgtechemail { get; set; }
        public string orgtechref { get; set; }
        public string orgabusehandle { get; set; }
        public string orgabusename { get; set; }
        public string orgabusephone { get; set; }
        public string orgabuseemail { get; set; }
        public string orgabuseref { get; set; }
        public string origin { get; set; }
        public string organization { get; set; }
        public string lastmodified { get; set; }
        public string techc { get; set; }
    }
}
